...
class RPG::Skill
  class Cost
    ...
    def arg_real(arg,subject)
      t, v = arg
      case t
      when :variable ; $game.variables[v]
      when :subject  ; subject.send(v)
      else           ; v
      end
    end
    ...
    def initialize(type,calc_type,*params)
      @type   = type || TYPE_MP
      @calc_type = calc_type || CALC_INT
      @params = params
    end

    def calc_cost(subject)
      case @calc_type
      when CALC_INT
    ...
    module Constants
      ...
      TYPE_MP   = 2   # // Default
      TYPE_TP   = 3   # //
      # // (type, CALC_INT, cost_int)
      # // (type, CALC_INT, [:variable,id])
      CALC_INT    = 1  # // Default
  ...
  def calc_hp_cost(subject)
    hp_cost.calc_cost(subject)
  end

  def calc_mp_cost(subject)
    mp_cost.calc_cost(subject)
  end

  def calc_tp_cost(subject)
    tp_cost.calc_cost(subject)
  end

  def hp_cost
    unless(@hp_cost.is_a?(Cost))
      @hp_cost = Cost.new(Cost::TYPE_HP,Cost::CALC_INT,[:nil,@hp_cost])
    end
    @hp_cost
  end
  ...
  def mp_cost
    unless(@mp_cost.is_a?(Cost))
      @mp_cost = Cost.new(Cost::TYPE_MP,Cost::CALC_INT,[:nil,@mp_cost])
    end
    @mp_cost
  end
  ...
  def tp_cost
    unless(@tp_cost.is_a?(Cost))
      @tp_cost = Cost.new(Cost::TYPE_TP,Cost::CALC_INT,[:nil,@tp_cost])
    end
    @tp_cost
  end
